...
$(function(){
    ...
    getData();
   	...
   	$("#addbutton").on("click",function(){
   		...
   		window.location.href=getIP()+"media/caipu/caipuAdd.jsp";
   	})
    ...
    $("#delbutton").on("click",function(){
    	...
    	if(confirm("确定要删除吗？")){
        	...
        	del.ajax(getIP()+"caipu.action?delMore",function(data){
        		if(data.success){
    ...
    function getData(){  
	   	...
	   	getPageData(sel,getIP()+"caipu.action?list",function(data){
				...
				if(len >0){
					$.each(data.rows,function(i,val){
						...
						html+="<tr>" +
								...
								"<td><img style='width:85px;' src="+getIP()+val.a6+"></img></td>" +
								"<td>"+
...
function update(id){
	 ...
	 window.location.href=getIP()+"media/caipu/caipuModify.jsp?eid="+id;
}
//修改
function jieshu(id){
